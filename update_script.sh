#!/bin/bash
# Apply diff 1: Update imports and remove type/const declarations
cat << 'DIFF' > patch1.diff
<<<<<<< SEARCH
...
=======
import { Volume2, VolumeX, Shield, Zap, RotateCcw, Tv, Crosshair, ArrowUp, ArrowDown, ArrowLeft, ArrowRight, Flame } from 'lucide-react';
import { TankGameState, Tank, TileType, EnemyType, PowerUpType, Direction } from './tank-defense/types';
...
node -e "
const fs = require('fs');
const diff = fs.readFileSync('patch1.diff', 'utf8');
const file = fs.readFileSync('src/components/arcade/TankDefenseCanvas.tsx', 'utf8');
const search = diff.split('<<<<<<< SEARCH\\n')[1].split('\\n=======\\n')[0];
const replace = diff.split('\\n=======\\n')[1].split('\\n>>>>>>> REPLACE')[0];
fs.writeFileSync('src/components/arcade/TankDefenseCanvas.tsx', file.replace(search, replace));
"

# Apply diff 2: Replace render logic
cat << 'DIFF' > patch2.diff
<<<<<<< SEARCH
            // ── RENDER SCENE ────────────────────────────────────────────────
            ctx.save();
...
node -e "
const fs = require('fs');
const diff = fs.readFileSync('patch2.diff', 'utf8');
const file = fs.readFileSync('src/components/arcade/TankDefenseCanvas.tsx', 'utf8');
const search = diff.split('<<<<<<< SEARCH\\n')[1].split('\\n=======\\n')[0];
const replace = diff.split('\\n=======\\n')[1].split('\\n>>>>>>> REPLACE')[0];
fs.writeFileSync('src/components/arcade/TankDefenseCanvas.tsx', file.replace(search, replace));
