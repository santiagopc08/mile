...
describe('TimelineCommentsDrawer Component', () => {
    ...
    it('handleDeleteComment should handle fetch error and show alert', async () => {
        ...
        // Wait for it to render fully (framer-motion AnimatePresence and Portal can delay)
        await waitFor(() => {
            expect(container.querySelector('button.hover\\:text-red-400')).toBeTruthy();
        }, { timeout: 2000 });

        const button = container.querySelector('button.hover\\:text-red-400');
