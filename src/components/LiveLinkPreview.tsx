...
function useUrlValidation(url: string) {
    ...
    return { debouncedUrl, isValid };
}

function useLinkPreviewFetch(debouncedUrl: string, isValid: boolean) {
...
function LiveLinkPreviewLoading() {
    return (
        <motion.div
...
function LiveLinkPreviewCard({ preview, debouncedUrl, label }: { preview: LinkPreviewData, debouncedUrl: string, label: string }) {
    let domain = 'ENLACE';
    try {
...
export function LiveLinkPreview({ url, label = 'VISTA PREVIA EN VIVO' }: LiveLinkPreviewProps) {
    const { debouncedUrl, isValid } = useUrlValidation(url);
    const { preview, loading } = useLinkPreviewFetch(debouncedUrl, isValid);

    if (!url.trim() || !isValid) return null;
    if (loading) return <LiveLinkPreviewLoading />;
    if (!preview) return null;

    return <LiveLinkPreviewCard preview={preview} debouncedUrl={debouncedUrl} label={label} />;
}
