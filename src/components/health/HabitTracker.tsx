    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {/* Input Section */}
                <div className="border border-white/10 bg-[#0a0a0a] p-6 relative">
                        ...
                        {(Object.keys(HABIT_CONFIG) as HealthHabitType[]).map(type => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`habit-btn p-4 flex flex-col items-center justify-center gap-3 ${selectedHabit === type ? 'ring-1 ring-white/50 bg-white/10' : ''}`}
                                >
                                    <config.icon className="w-6 h-6" style={{ color: config.color }} />
                                    <span className="text-[9px] font-bold uppercase tracking-wider text-white/70 text-center">{config.label}</span>
                                </button>
                            );
                        })}
                    </div>
                                ...
                                <div className="space-y-4 pt-4 border-t border-white/10">
                                    <div className="space-y-2">
                                        ...
                                        <input 
                                            ...
                                            className="w-full bg-black border border-white/10 p-3 text-sm font-mono text-white outline-none focus:border-white/30"
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            <button type="button" onClick={() => setSeverity('high')} className={`flex-1 py-2 text-[9px] uppercase font-bold border transition-colors ${severity === 'high' ? 'border-[#ff003c] text-[#ff003c] bg-[#ff003c]/10' : 'border-white/10 text-white/30'}`}>Alta</button>
                                        </div>
                                    </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="border border-white/10 bg-[#050505] p-6">
                        <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-[#a88a7e] mb-4 flex items-center gap-2">
                            <TrendingDown className="w-4 h-4" /> Impacto Financiero
                        </h3>
                                ...
                                return (
                                    <div key={type}>
                                        ...
                                        <div className="h-1.5 w-full bg-white/5">
                                            <div className="h-full" style={{ width: `${percentage}%`, backgroundColor: config.color }} />
                                        </div>
                                    </div>
                                );
                            })}
                    ...
                    {/* Recent History */}
                    <div className="border border-white/10 bg-black p-4 max-h-[300px] overflow-y-auto custom-scrollbar">
                        <h3 className="text-[9px] font-black uppercase tracking-[0.2em] text-white/30 mb-3 sticky top-0 bg-black pb-2">
                            Historial Reciente
                        </h3>
                        <div className="space-y-2">
                            {habits.filter(h => h.profile === profile).slice(0, 10).map(h => {
                                ...
                                return (
                                    <div key={h.id} className="flex items-center justify-between p-2 border border-white/5 bg-white/[0.02]">
                                        <div className="flex items-center gap-3">
                                            <div className={`w-1.5 h-1.5 rounded-full ${h.severity === 'high' ? 'bg-[#ff003c]' : h.severity === 'medium' ? 'bg-[#ffb595]' : 'bg-[#c3f400]'}`} />
                                            <div>
                                                <div className="text-[10px] font-bold uppercase tracking-wider text-white/80">{config?.label}</div>
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <button onClick={() => handleDelete(h.id)} className="text-[8px] text-white/20 hover:text-red-400">✕</button>
                                        </div>
                                    </div>
                                );
                            })}
